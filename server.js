//Crear servidor
const net = require('net'); //commont js
...
const configDensidad = async (ip, modelo, densidad,sendEvent) => {
  if (modelo == "MFP E62555" || "MFP E62655" || "Laser E60155" || "Color MFP E67550" || "Color MFP E67650") {
    //Navegador
    sendEvent('Iniciando configuración de densidad...');
    ...
    try {
      const header = randomUseragent.getRandom();
      await page.setUserAgent(header);
      sendEvent('Navegador configurado');
      await page.setViewport({ width: 1920, height: 1080 });
      sendEvent(`Navegando a la página del dispositivo con IP: ${ip}`);
      sendEvent('Verificando versión de firmware...');

      sendEvent('Navegando URL versión Firmware 4.12');
      await page.goto(
      ...
      if (selectorExiste) {
        sendEvent('Navegando URL versión Firmware 5.7');
        await page.goto(
          "https://" + ip + "/hp/device/DefaultPrintOptions/Index", // firmawre 5.7.1
          { timeout: 15000 }
        );
      }

      sendEvent('Iniciando sesión en modo administrador...');
      ...
      switch (densidad) {
        case "0":
          densidadAjustar = "1";
          sendEvent('Densidad establecida al 0%');
          break;
        case "25":
          densidadAjustar = "2";
          sendEvent('Densidad establecida al 25%');
          break;
        case "50":
          densidadAjustar = "3";
          sendEvent('Densidad establecida al 50%');
          break;
        case "75":
          densidadAjustar = "4";
          sendEvent('Densidad establecida al 75%');
          break;
        default:
          densidadAjustar = "5";
          sendEvent('Densidad establecida al 100%');
          break;
      }
      ...
      try {
        await page.waitForSelector('#Summary.message-success', { timeout: 10000 }); // Espera hasta 10 segundos por el mensaje de éxito
        ...
        if (successMessage.includes("La operación ha finalizado correctamente.")) {
          sendEvent("Densidad configurada, la operación ha finalizado correctamente.");
          await browser.close();
          return "Configuración de densidad exitosa";
        } else {
          sendEvent("Densidad No Configurada: El mensaje de éxito no coincide.");
          await browser.close();
          return "Error al configurar la densidad.";
        }

      } catch (e) {
        console.log("Configuración fallida: No se encontró el mensaje de éxito se demoro en responder la pagina.");
        await browser.close();
        sendEvent('Configuración fallida: No se encontró el mensaje de éxito se demoro en responder la pagina.');
      }
      ...
      sendEvent('Error la pagina no se cargo correctamente.');
      console.error("Error la pagina no se cargo correctamente. " + error.message);
      sendEvent('Error al configurar la densidad');
      return "Error al configurar la densidad";
    }

  } else {
    sendEvent('Modelo no válido');
...
// FUNCIÓN CONFIG INFO
const configInfo = async (ip, modelo, namePrint, printLocation,sendEvent) => {
  if (modelo == "MFP E62555" || "MFP E62655" || "Laser E60155" || "Color MFP E67550" || "Color MFP E67650") {
    //Navegador
    sendEvent('Iniciando configuración de información basica...');
    const browser = await puppeteer.launch({
      executablePath:
        "C:\\Program Files (x86)\\Microsoft\\Edge\\Application\\msedge.exe",
      headless: true,
    ...
    try {
      ...
      sendEvent('Ubicación del dispositivo: '+printLocation+' OK');
      ...
      sendEvent('Numero de activo: '+serialNumber+' OK');
      ...
      sendEvent('Nombre de la empresa: '+nombreEmpresa+' OK');
      ...
      sendEvent('Persona de contacto: '+personContac+' OK');
      ...
      try {
        ...
        if (successMessage.includes("La operación ha finalizado correctamente.")) {
          sendEvent("Información configurada, la operación ha finalizado correctamente.");
          return "Información Actualizada";
        } else {
          sendEvent("Información No Actualizada: El mensaje de éxito no coincide.");
          return "Información No Actualizada: El mensaje de éxito no coincide.";
        }
      ...
      console.error("Error la pagina no se cargo correctamente. " + error.message);
      await browser.close();
      sendEvent('Error al configurar información');
...
// FUNCIÓN CONFIG INFO
const configCredenciales = async (ip, modelo, sendEvent) => {
  if (modelo == "MFP E62555" || "MFP E62655" || "Laser E60155" || "Color MFP E67550" || "Color MFP E67650") {
    //Navegador
    sendEvent('Iniciando configuración de credenciales Administrador...');
    const browser = await puppeteer.launch({
      executablePath:
        "C:\\Program Files (x86)\\Microsoft\\Edge\\Application\\msedge.exe",
      headless: true,
    ...
    try {
      ...
      sendEvent(`Navegando a la página del dispositivo con IP: ${ip}`);
      await page.goto(
        "https://" + ip + "/hp/device/GeneralSecurity/Index",
        { timeout: 20000 }
      );

      //validar espera aqui veroificar y enviar al servidor si ingresa y entra con la contraseña normal es por que ya la tiene configurada
      try {
        //validar espera
      sendEvent("Validando si las credenciales estan configuradas...");
      const ipInput = await page.waitForSelector("#PasswordTextBox", {
        timeout: 15000,
      });
      ...
      console.log("La impresora ya tiene configurado el usuario administrador con la contraseña correcta");
      await browser.close();
      sendEvent('Si, Credenciales ya configuradas.');
      return "Impresora Ya configurada con credenciales correctas."
      } catch (error) {
        sendEvent('No, Configurando Credenciales...');
        const ipInput = await page.waitForSelector("#newPassword", {
          timeout: 15000,
        });
        await page.waitForSelector("#content", { timeout: 1000 });
        await page.click("#newPassword");
        await page.type('#newPassword', pass.toString());
        await page.waitForTimeout(1000); // Espera 500
        await page.click("#verifyPassword");
        await page.type('#verifyPassword', pass.toString());
        await page.waitForTimeout(2000); // Espera 500
        await page.click("#FormButtonSubmit");
        sendEvent("Esperando mensaje de confirmación...");
        try {
          ...
          if (successMessage.includes("La operación ha finalizado correctamente.")) {
            sendEvent("Credenciales configuradas, la operación ha finalizado correctamente.");
            return "Configuración de credenciales exitosa";
          } else {
            sendEvent("Configuración No Actualizada: El mensaje de éxito no coincide.");
            return "Configuración No Actualizada: El mensaje de éxito no coincide.";
          }

        } catch (e) {
          console.log("Configuración fallida: No se encontró el mensaje de éxito se demoro en responder la pagina."+ e);
          sendEvent("Configuración fallida: No se encontró el mensaje de éxito se demoro en responder la pagina.");
        }
      ...
      await browser.close();
      sendEvent('Error al configurar credenciales');
...
const configCorreo = async (ip, modelo, sendEvent) => {
  if (modelo == "MFP E62555" || "MFP E62655" || "Laser E60155" || "Color MFP E67550" || "Color MFP E67650") {
    //Navegador
    sendEvent('Iniciando configuración de correo...');
    const browser = await puppeteer.launch({
      executablePath:
        "C:\\Program Files (x86)\\Microsoft\\Edge\\Application\\msedge.exe",
      headless: false,
    ...
    try {
      ...
      sendEvent('Navegador configurado');
      await page.setViewport({ width: 1920, height: 1080 });
      sendEvent(`Navegando a la página de la impresora con IP: ${ip}`);
      await page.goto(
      ...
      //click
      sendEvent('Iniciando sesión...');
      await page.click("#PasswordTextBox");
      await ipInput.type(pass.toString());
      await page.click("#signInOk");
      await page.waitForSelector("#menuTabs", { timeout: 2000 });
      await page.click("#SendPages");
      sendEvent('Sesión iniciada. Accediendo al menú...');

      sendEvent('Estableciendo variables predeterminadas...');
      ...
      //FIN CONFIG CORREO
      console.log("La página se cargó correctamente.");
      sendEvent('Configuración completada');
      await browser.close();
      return "Configuración email exitosa";

    } catch (error) {
      console.error("Error la pagina no se cargo correctamente. " + error.message);
      sendEvent('Error durante la configuración');
      await browser.close();
...
const configRed = async (ip, modelo) => {
  if (modelo == "MFP E62555" || "MFP E62655" || "Laser E60155" || "Color MFP E67550" || "Color MFP E67650") {
    ...
    try {
      ...
      await page.goto(
        "https://" + ip + "/hp/device/DeviceStatus/Index",
        { timeout: 20000 }
      );
...
async function generarReporte() {
  try {
    ...
    facturacion.sheet(0).cell("I1").value('CONTADOR FINAL');
    facturacion.sheet(0).cell("J1").value('CONTADOR INICIAL');
    ...
    //Salida Documento
    //await facturacion.toFileAsync('C:\\Users\\efacamayo.TQCLO01\\OneDrive - Carvajal S.A\\1. Operación Carvajal\\13. Contadores\\Consolidado\\' + formattedDate + 'PRUEBAS.xlsx');
    await facturacion.toFileAsync('C:\\Users\\efacamayo.TQCLO01\\OneDrive - Carvajal S.A\\1. Operación Carvajal\\13. Contadores\\Consolidado\\' + formattedDate + '.xlsx');
...
app.post('/enviar-ip-densidad-modelo', async (req, res) => {
  ...
  const clientId = req.body.clientId; // Recibe el ID del cliente desde el cliente
  ...
  try {
    const resultado = await configDensidad(ip, modelo, densidad,(message)=>{
      sendEventToClient(clientId,message);
    });
...
app.post('/enviar-ip-modelo-namePrint-location', async (req, res) => {
  ...
  try {
    const resultado = await configInfo(ip, modelo, namePrint, printLocation,(message)=>{
      sendEventToClient(clientId,message);
    });
    res.status(200).json({ message: resultado });
  } catch (error) {
...
//CODIGO ACTUAL CONFIGCREDENCIALES
app.post('/configCredenciales', async (req, res) => {
  ...
  try {
    // Llamar a la función configCredenciales y pasar la función sendEventToClient
    const resultado = await configCredenciales(ip, modelo, (message) => {
      sendEventToClient(clientId, message); // Enviar eventos solo al cliente correspondiente
    });
    // Enviar respuesta final de la solicitud POST al cliente
    res.status(200).json({ message: resultado });
  } catch (error) {
...
app.post('/configCorreo', async (req, res) => {
  ...
  try {
    const resultado = await configCorreo(ip, modelo,(message)=>{
      // Lógica para enviar eventos (puedes almacenar mensajes en tiempo real aquí si lo deseas)
...
// Configurar la tarea con cron para ejecutarse a las 11:00 p.m. todos los días
cron.schedule('0 23 * * *', async () => { // Minutos Hora
  console.log('Iniciando la ejecución programada del reporte de contadores');
  try {
    await generarReporte();
  } catch (error) {
    console.error('Error durante la ejecución del programa', error);
  }
});

// MANEJAR EVENTOS DE FORMA UNIDIRECCIONAL EN TIEMPO REAL CON SSE
// Ruta SSE para enviar mensajes en tiempo real al cliente
// Mantén un objeto para gestionar las conexiones SSE de cada cliente
// Objeto para almacenar las conexiones SSE de cada cliente por ID
let clients = {};
// Ruta GET para establecer la conexión SSE
app.get('/events/:id', (req, res) => {
  ...
  // Almacenar la conexión del cliente en el objeto clients
  clients[clientId] = res;

  // Enviar un mensaje inicial al cliente
  res.write(`data: ${JSON.stringify({ message: '---> Conexión establecida con el servidor, inicio del proceso.' })}\n\n`);

  // Manejar la desconexión del cliente
  req.on('close', () => {
    console.log(`Conexión cerrada con cliente ${clientId}`);
    delete clients[clientId]; // Remover al cliente al cerrar la conexión
  });
});

// Función para enviar eventos a un cliente específico por ID
const sendEventToClient = (clientId, message) => {
  if (clients[clientId]) {
    clients[clientId].write(`data: ${JSON.stringify({ message })}\n\n`);
  } else {
    console.log(`Cliente con ID ${clientId} no está conectado.`);
  }
};